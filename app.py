...
# -------------------------------------------------
# PLOTTING (smaller charts + metrics panel)
# -------------------------------------------------
def plot_core(df: pd.DataFrame, location: str, day: str):
    metrics = compute_metrics(df)

    # ~1/4 the original area (18x10 -> 9x5)
    ...
    # --- Metrics panel (top-right) - NO overlap with chart
    ...
    lines = [
        ...
        f"Mean Err: ${metrics['mean_error']:.2f}",
    ...
    # --- Error histogram (bottom full width)
    ax2 = fig.add_subplot(gs[1, :])
    ax2.hist(df["error"], bins=25, alpha=0.85)
...
def plot_advanced(df: pd.DataFrame, location: str, day: str, threshold: float):
    ...
    # smaller than before (18x11 -> ~9x5.5)
...
# -------------------------------------------------
# STREAMLIT APP (SIDEBAR CONTROLS)
# -------------------------------------------------
